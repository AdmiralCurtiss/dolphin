...
void SkylanderFigure::Encrypt(std::span<u8, FIGURE_SIZE> input)
{
  std::array<u8, 0x56> hash_in = {};
...
bool SkylanderFigure::Create(u16 m_sky_id, u16 m_sky_var)
{
  ...
  memcpy(&m_data[0x10], &m_sky_id, sizeof(m_sky_id));
...
FigureData SkylanderFigure::GetData() const
{
  ...
  if (figure_data.normalized_type == Type::Skylander)
  {
    ...
    memcpy(figure_data.skylander_data.nickname.data(), nickname, 2 * 16);
  }
...
void SkylanderFigure::SetData(FigureData* figure_data)
{
  ...
  if (figure_data->normalized_type == Type::Skylander)
  {
...

    {
      ...
      if (decrypted[0x112] == decrypted[0x2D2])
        decrypted[area_offset + 0x2]++;
      else
  ...
  else if (figure_data->normalized_type == Type::Trophy)
  {
    ...
    {
      u16 area_offset = (decrypted[0x112] != (decrypted[0x2D2] + 1U)) ? 0x110 : 0x2D0;
...
void SkylanderFigure::DecryptFigure(u8 dest[FIGURE_SIZE]) const
{
  std::array<u8, 0x56> hash_in = {};
  ...
  memcpy(dest, decrypted.data(), FIGURE_SIZE);
