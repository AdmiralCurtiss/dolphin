...
{
  auto figure = std::make_unique<IOS::HLE::USB::SkylanderFigure>(
      IOS::HLE::USB::SkylanderFigure(path.toStdString()));
...
{
  File::IOFile m_sky_file(path.toStdString(), "r+b");
  if (!m_sky_file)
  {
    QMessageBox::warning(
  ...
  std::array<u8, 0x40 * 0x10> file_data;
  if (!m_sky_file.ReadBytes(file_data.data(), file_data.size()))
  {
    QMessageBox::warning(
  ...
  const u8 portal_slot =
      system.GetSkylanderPortal().LoadSkylander(std::make_unique<IOS::HLE::USB::SkylanderFigure>(
          IOS::HLE::USB::SkylanderFigure(std::move(m_sky_file))));
  if (portal_slot == 0xFF)
  {
...
QString SkylanderPortalWindow::GetFilePath(u16 id, u16 var)
{
  ...
  for (const auto& file : collection.entryInfoList(QStringList(QStringLiteral("*.sky"))))
  {
    File::IOFile m_sky_file(file.filePath().toStdString(), "r+b");
    if (!m_sky_file)
    {
      continue;
    }
