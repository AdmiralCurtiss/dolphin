...
MemArena::MemArena()
{
  ...
  HMODULE kernelBaseHandle = LoadLibraryW(L"KernelBase.dll");
  if (!kernelBaseHandle)
    return;

  void* ptr_IsApiSetImplemented = GetProcAddress(kernelBaseHandle, "IsApiSetImplemented");
  if (ptr_IsApiSetImplemented &&
      static_cast<PIsApiSetImplemented>(ptr_IsApiSetImplemented)("api-ms-win-core-memory-l1-1-6"))
  {
    HMODULE handle = LoadLibraryW(L"api-ms-win-core-memory-l1-1-6.dll");
    if (handle)
    {
      void* ptr_VirtualAlloc2 = GetProcAddress(handle, "VirtualAlloc2FromApp");
      void* ptr_MapViewOfFile3 = GetProcAddress(handle, "MapViewOfFile3FromApp");
      if (ptr_VirtualAlloc2 && ptr_MapViewOfFile3)
      {
        m_api_ms_win_core_memory_l1_1_6_handle = handle;
        m_address_VirtualAlloc2 = ptr_VirtualAlloc2;
        m_address_MapViewOfFile3 = ptr_MapViewOfFile3;
      }
      else
      {
...
void* MemArena::MapInMemoryRegion(s64 offset, size_t size, void* base)
{
  if (m_api_ms_win_core_memory_l1_1_6_handle)
  {
    ...
    if (rv)
    {
      ...
      NOTICE_LOG_FMT(MEMMAP, "Mapped memory region, now tracking {} parts.", m_regions.size());
    }
    else
    {
